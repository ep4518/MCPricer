...
class BSModel(Model):
    ...
    def GenerateSamplePath(self, T: float, m: int, S: SamplePath):
        St = self.S0
        for i in range(m):
            S[i] = St * exp((self.r - self.sigma * self.sigma * 0.5) * (T / m)) + self.sigma * sqrt(T / m) * Gauss()
            St = S[i]
            
# https://quant-next.com/the-merton-jump-diffusion-model/

class MJDModel(Model):
    def __init__(self, S0, r: float, sigma: float, mu: float, lambda_: float, mean: float, delta: float):
        super().__init__(S0=S0, r=r, sigma=sigma)
        self.mu = mu 
    ...
    def normal_compound_poisson_process(self, T):
        ...
        while t < T:
            ...
            jumps = jumps + normal(self.mean, self.delta) #m + Gauss() * self.delta     # N(m, delta)
            if t < T:
    ...
    def GenerateSamplePath(self, T: float, m: int, S: SamplePath):
        dt = T / m
        St = self.S0
        k = exp(self.mean + .5 * self.delta**2) - 1
        
        for i in range(m):
            dW = sqrt(dt) * Gauss()
            jumps = sum(self.normal_compound_poisson_process(dt))
            # print(St, self.mu, self.sigma, self.lambda_, k, jumps)
            S[i] = St * exp((self.mu - 0.5 * self.sigma ** 2 - self.lambda_ * k) * dt + self.sigma * dW + jumps)
            St = S[i]